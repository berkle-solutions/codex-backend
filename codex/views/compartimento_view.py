...
from codex.serializers.compartimento_serializer import Compartimento, CompartimentoSerializer
...
class CompartimentoView(APIView):
    ...
    def retorna_compartimentos_por_armario(request, armario_id):
        try:
            compartimentos_por_armario = Compartimento.objects.filter(armario_id=armario_id)
            compartimentos_por_armario_serializer = CompartimentoSerializer(compartimentos_por_armario, many=True)
    ...
    def atualizar_compartimento(request, pk):
        try:
            compartimento = Compartimento.objects.get(pk=pk)
            compartimento_serializer = CompartimentoSerializer(compartimento, data=request.data)
            if compartimento_serializer.is_valid():
                compartimento_serializer.save()
                return Response(compartimento_serializer.data)
            return Response(compartimento_serializer.errors, status=status.HTTP_400_BAD_REQUEST)
